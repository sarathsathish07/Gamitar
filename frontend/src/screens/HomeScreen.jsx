...
import { useFetchGridQuery, useUpdateGridCellMutation } from '../slices/usersApiSlice';
...
const HomeScreen = () => {
  ...
  const handleCellClick = async (rowIndex, colIndex) => {
    ...
    if (char && char.length === 1) {
      try {
        const response = await updateGridCell({ rowIndex, colIndex, unicodeChar: char });
  
        if (response.error) {
          if (response.error.status === 403) {
            toast.error('You have already updated a cell and cannot update another.');
  ...
  return (
    <Row>
      ...
      <h2 className='mt-2'>Unicode Character Grid</h2>
...
<Col md={4}>
<div className="d-flex justify-content-center users-online my-5">
          <h4 className='p-4'>Users Online: {onlineUsers}</h4>  
        </div>
</Col>
    </Row>
