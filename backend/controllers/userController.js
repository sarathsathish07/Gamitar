...
const updateGrid = expressAsyncHandler(async (req, res, next) => {
  ...
  if (grid) {
    const userHasUpdated = grid.grid.some(row => 
      row.some(cell => cell !== null && cell.userId && cell.userId.toString() === userId.toString())
    );

    if (userHasUpdated) {
      res.status(403).json({ message: 'You have already updated a cell and cannot update another.' });
      return;
    }

    if (!grid.grid[rowIndex][colIndex] || grid.grid[rowIndex][colIndex] === null) {
